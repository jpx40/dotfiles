(() => {
  ...
  function resetAllInRawStorage(callback) {
    return mutex.lock(async () => {
      const oldItems = await loadAllFromRawStorage();
      const newItems = callback(oldItems);
      await browser_browser.storage.local.clear();
      await browser_browser.storage.local.set(newItems);
    });
  }
  ...
  async function updateAll() {
    ...
    void browser_browser.alarms.create(UPDATE_ALL_ALARM_NAME, { periodInMinutes: updateInterval });
  ...
  async function restore(items) {
    await resetAllInRawStorage(({ nextSubscriptionId }) => {
      ...
      const now = esm().toISOString();
      const blacklist = (_a = items.blacklist) != null ? _a : defaults.blacklist;
      ...
      return {
        ...
        compiledRules,
        timestamp: now,
        blockWholeSite: (_b = items.blockWholeSite) != null ? _b : defaults.blockWholeSite,
        skipBlockDialog: (_c = items.skipBlockDialog) != null ? _c : defaults.skipBlockDialog,
        hideBlockLinks: (_d = items.hideBlockLinks) != null ? _d : defaults.hideBlockLinks,
        hideControl: (_e = items.hideControl) != null ? _e : defaults.hideControl,
        enablePathDepth: (_f = items.enablePathDepth) != null ? _f : defaults.enablePathDepth,
        generalLastModified: now,
        linkColor: (_g = items.linkColor) != null ? _g : defaults.linkColor,
        blockColor: (_h = items.blockColor) != null ? _h : defaults.blockColor,
        highlightColors: (_i = items.highlightColors) != null ? _i : defaults.highlightColors,
        dialogTheme: (_j = items.dialogTheme) != null ? _j : defaults.dialogTheme,
        appearanceLastModified: now,
        syncCloudId: false,
        syncCloudToken: false,
        syncResult: false,
        syncBlocklist: (_k = items.syncBlocklist) != null ? _k : defaults.syncBlocklist,
        syncGeneral: (_l = items.syncGeneral) != null ? _l : defaults.syncGeneral,
        syncAppearance: (_m = items.syncAppearance) != null ? _m : defaults.syncAppearance,
        syncSubscriptions: (_n = items.syncSubscriptions) != null ? _n : defaults.syncSubscriptions,
        syncInterval: (_o = items.syncInterval) != null ? _o : defaults.syncInterval,
        subscriptions,
        nextSubscriptionId,
        updateInterval: (_p = items.updateInterval) != null ? _p : defaults.updateInterval,
  ...
  async function initialize() {
    ...
    const matches = utilities_stringEntries(search_engines_SEARCH_ENGINES).flatMap(
      ([id, { contentScripts }]) =>
        id !== 'google' ? contentScripts.flatMap(({ matches: matches2 }) => matches2) : [],
    );
    await browser_browser.permissions.remove({ origins: matches });
  }
...
--${MULTIPART_RELATED_BOUNDARY}--`,
  ...
  const syncSections = [
    ...
    },
    {
      ...
      afterDownload(cloudItems, cloudContent, cloudModifiedTime) {
        const items = parse(
          cloudContent,
          type({
            skipBlockDialog: microstruct_boolean(),
            hideBlockLinks: microstruct_boolean(),
            hideControl: microstruct_boolean(),
            enablePathDepth: microstruct_boolean(),
      ...
      afterDownload(cloudItems, cloudContent, cloudModifiedTime) {
        const items = parse(
          ...
          type({
            ...
            dialogTheme: enums(['light', 'dark', 'default']),
          }),
        );
        if (!items) {
    ...
    },
    {
      ...
      afterDownload(cloudItems, cloudContent, cloudModifiedTime, localItems) {
        const items = parse(
          cloudContent,
          array(type({ name: string(), url: string(), enabled: optional(microstruct_boolean()) })),
        );
        if (!items) {
  ...
  async function doSync(dirtyFlags2, repeat) {
    return sync_mutex.lock(async () => {
      ...
      if (repeat) {
        void browser_browser.alarms.create(SYNC_ALARM_NAME, {
